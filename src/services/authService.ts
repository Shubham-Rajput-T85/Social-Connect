...
export const login = async (res:Response, loginObj: loginDTO) => {
    const userObj = await User.findOne({
      ...
      }).select("_id name username bio profileUrl email hashPassword isPrivate postCount followersCount followingCount storyCount");
...
export const getMe = async (token: string) => {
    ...
    const user = await User.findById(decoded.userId).select("_id name username bio profileUrl email isPrivate postCount followersCount followingCount storyCount");
    return user;  
}
